...
-- 	IF EXISTS (SELECT 1 FROM tb_User p WHERE Email = pEmail)
-- 	THEN
-- 		-- Account attempt less than 5
-- 		IF EXISTS (SELECT 1 FROM tb_User p WHERE Email = pEmail AND p.Attempt < 5)
-- 		THEN
-- 			-- Valid credentials
-- 			IF EXISTS (SELECT 1 FROM tb_User p WHERE Email = pEmail AND Password = pPassword)
-- 			THEN
...
BEGIN
    ...
    -- Check if the target tile is emptied
    ...
    THEN
        ...
        IF EXISTS (SELECT 1 FROM tb_Inventory_Item WHERE UserID = pUserID AND ItemTypeID = current_item_type)
        THEN
        ...
        ELSE
            ...
            INSERT INTO tb_Inventory_Item (UserID, ItemTypeID, Quantity) VALUES (pUserID, current_item_type, 1);
        END IF;
    END IF;
END//
DELIMITER ;
